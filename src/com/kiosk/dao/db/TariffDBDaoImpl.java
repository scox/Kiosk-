...
public class TariffDBDaoImpl extends BaseDao implements TariffDBDao {
	...
	public int deleteTariff(int tariffID) {
		return getJdbcTemplate().update("DELETE from price where price_id = ?",
				new Object[] { tariffID });
	}
	...
	public Tariff getIndividualTariff(int tariffID) {
		...
		List<Tariff> tariffs = getJdbcTemplate()
				.query("SELECT price_ID,price, audio_level FROM price where price_ID = ?",
						new Object[] { tariffID }, new RowMapper<Tariff>() {
	...
	public int updateTariff(int id, Double price, String level) {
		return getJdbcTemplate()
				.update("update price set price = ?, audio_level =? where price_id = ?",
						new Object[] { price, level, id });
	}
	...
	public int addTariff(String level, Double price) {
		...
		return getJdbcTemplate().update(
				"insert into price(audio_level, price) values (?,?)",
				new Object[] { level, price });
	}
